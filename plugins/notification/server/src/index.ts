...
export function createNotificationPlugin(config: NotificationPluginConfig): NotificationPlugin {
  ...
  const sendFromTemplateFn = async (
    ...
    try {
      const { body, subject } = await renderTemplate(
        params.templateKey,
        variablesToObject(params.variables),
      )
