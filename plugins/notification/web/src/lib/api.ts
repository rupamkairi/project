...
const API_URL = (import.meta as any).env?.VITE_API_URL || 'http://localhost:10050'
  ...
  notifications: {
    templates: {
      ...
      ':key': {
        get: async (options: { params: { key: string } }) => {
          const res = await fetch(`${API_URL}${BASE_PATH}/templates/${options.params.key}`)
          return { data: await res.json() }
        },
        patch: async (
    ...
    scheduled: {
      get: async (options?: { query?: { status?: string } }) => {
        const query = options?.query?.status ? `?status=${options.query.status}` : ''
        const res = await fetch(`${API_URL}${BASE_PATH}/scheduled${query}`)
        return { data: await res.json() }
      },
      ':id': {
        get: async (options: { params: { id: string } }) => {
          const res = await fetch(`${API_URL}${BASE_PATH}/scheduled/${options.params.id}`)
          return { data: await res.json() }
        },
        patch: async (
...
export const templatesApi = {
  list: async (params?: { channel?: string; locale?: string; isSystem?: boolean }) => {
  ...
  create: async (data: Partial<Template>) => notificationApi.notifications.templates.post(data),
...
export const scheduledApi = {
  ...
  get: async (id: string) => notificationApi.notifications.scheduled[':id'].get({ params: { id } }),
