...
export function createStoragePlugin(config: StoragePluginConfig): StoragePlugin {
  ...
  const deleteFile = async (params: { fileId: string; organizationId: string }): Promise<void> => {
    ...
    await db.update(storageFiles).set({ deletedAt: new Date() }).where(eq(storageFiles.id, fileId))
