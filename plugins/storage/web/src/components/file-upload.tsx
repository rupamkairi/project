...
import { Upload, File, X, Image, Loader2, FileText, Film, Music, Archive } from 'lucide-react'
import { Button, Spinner, Card, CardContent, cn } from '@projectx/ui'
import type { StorageApi } from '../lib/api'
...
export function FileUpload({
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      const { uploadUrl, fileId, key } = await api.getUploadUrl(file.name, file.type, folder)
    ...
    <div className="space-y-4">
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          ...
          {files.map((file) => (
            <FileItem key={file.id} file={file} onDelete={handleDelete} onPreview={handlePreview} />
          ))}
        </div>
      )}
    </div>
