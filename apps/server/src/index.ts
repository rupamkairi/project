...
const coreLayer = [
  ...
  },
  {
    ...
    types: ['RuleEngine', 'RuleExpr', 'RuleExplanation', 'createRuleEngine', 'Op'],
    filePath: './core/rule',
  },
  {
    id: 'cqrs',
    name: 'CQRS',
    description: 'Command Query Responsibility Segregation - mediator and handlers',
    types: [
      'Command',
      'Query',
      'CommandHandler',
  ...
  },
  {
    ...
    types: ['Queue', 'Worker', 'Scheduler', 'Job', 'JobOptions', 'JobStatus', 'BulkJob'],
    filePath: './core/queue',
  },
  {
    id: 'module',
...
const dbSchemas = [
  {
    id: 'identity',
    name: 'Identity',
    tables: ['organizations', 'actors', 'roles', 'actor_roles', 'sessions', 'api_keys'],
    filePath: './infra/db/schema/identity',
  },
  {
    id: 'catalog',
    name: 'Catalog',
    tables: ['cat_categories', 'cat_items', 'cat_variants', 'cat_price_lists', 'cat_price_rules'],
  ...
  },
  {
    ...
    tables: [
      ...
      'workplace_timesheet_entries',
      'workplace_goals',
      'workplace_review_cycles',
      'workplace_reviews',
      'workplace_review_criteria',
      'workplace_feedback',
      'workplace_pay_components',
      'workplace_salary_structures',
      'workplace_employee_compensation',
      'workplace_payroll_runs',
      'workplace_payslips',
      'workplace_expense_claims',
      'workplace_expense_items',
      'workplace_assets',
    ...
    filePath: './composes/workplace/server/src/db/schema/workplace',
  },
  {
    ...
    filePath: './composes/project-management/server/src/db/schema/project-management',
  },
  {
    id: 'hospitality',
    name: 'Hospitality',
    ...
    filePath: './composes/hospitality/server/src/db/schema/hospitality',
  },
]
...
async function main() {
  ...
  const authPlugin = createAuthPlugin(createPlatformAuthConfig(mediator))
  ...
  const { createWorkplaceCompose } = await import('@projectx/workplace-server')
  const workplaceCompose = createWorkplaceCompose(mediator, bus, bootRegistry.scheduler)

  const { createProjectManagementCompose } = await import('@projectx/project-management-server')
  const projectManagementCompose = createProjectManagementCompose(mediator)

  const { createHospitalityCompose } = await import('@projectx/hospitality-server')
  const hospitalityCompose = createHospitalityCompose(mediator)
  let app: any = new Elysia()
    // Plugins
    .use(cors())
    .use(swagger())
    .use(bearer())
    // One shared Platform session protects every mounted operational Compose.
    ...
    // Workplace Compose plugin
    .use(workplaceCompose)
    // Project Management Compose plugin
    .use(projectManagementCompose)
    // Hospitality Compose plugin
    .use(hospitalityCompose)
    // Health check
    .get('/health', () => ({
      status: 'ok',
