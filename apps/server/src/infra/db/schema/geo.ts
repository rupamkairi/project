...
export const geoEntities = pgTable(
  ...
  (table) => [
    ...
    index('geo_entities_org_geometry_type_idx').on(table.organizationId, table.geometryType),
  ],
)
...
export const geoTerritories = pgTable(
  ...
  (table) => [
    ...
    index('geo_territories_org_active_idx').on(table.organizationId, table.isActive),
  ],
)
