...
export const invLocations = pgTable(
  ...
  (table) => [
    index('inv_locations_org_default_idx').on(table.organizationId, table.isDefault),
...
export const invStockUnits = pgTable(
  ...
  (table) => [
    ...
    index('inv_stock_units_org_location_idx').on(table.organizationId, table.locationId),
    index('inv_stock_units_org_variant_idx').on(table.organizationId, table.variantId),
  ],
)
...
export const invMovements = pgTable(
  ...
  (table) => [
    index('inv_movements_org_variant_idx').on(table.organizationId, table.variantId),
    index('inv_movements_org_reference_idx').on(
      table.organizationId,
      table.referenceId,
      table.referenceType,
    ),
    index('inv_movements_org_reason_idx').on(table.organizationId, table.reason),
