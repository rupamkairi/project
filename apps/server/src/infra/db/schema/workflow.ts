...
export const wfProcessTemplates = pgTable(
  ...
  (table) => [
    index('wf_process_templates_org_entity_type_idx').on(table.organizationId, table.entityType),
    index('wf_process_templates_org_active_idx').on(table.organizationId, table.isActive),
  ],
)
...
export const wfProcessInstances = pgTable(
  ...
  (table) => [
    ...
    index('wf_process_instances_org_status_idx').on(table.organizationId, table.status),
    index('wf_process_instances_org_template_idx').on(table.organizationId, table.templateId),
  ],
)
...
export const wfTasks = pgTable(
  ...
  (table) => [
    index('wf_tasks_org_instance_idx').on(table.organizationId, table.instanceId),
