...
export const txStatusEnum = pgEnum('ldg_tx_status', ['pending', 'posted', 'voided'])
...
export const ldgAccounts = pgTable(
  ...
  (table) => [
    uniqueIndex('ldg_accounts_org_code_idx').on(table.organizationId, table.code),
    index('ldg_accounts_org_type_idx').on(table.organizationId, table.type),
    index('ldg_accounts_org_parent_idx').on(table.organizationId, table.parentId),
  ],
)
...
export const ldgTransactions = pgTable(
  ...
  (table) => [
    ...
    index('ldg_transactions_org_status_idx').on(table.organizationId, table.status),
    index('ldg_transactions_org_posted_idx').on(table.organizationId, table.postedAt),
  ],
)
...
export const ldgJournalEntries = pgTable(
  ...
  (table) => [
    ...
    index('ldg_journal_entries_org_account_idx').on(table.organizationId, table.accountId),
  ],
)
