...
export const ntfLogStatusEnum = pgEnum('ntf_log_status', ['pending', 'sent', 'failed', 'read'])
...
export const ntfTriggers = pgTable(
  ...
  (table) => [
    index('ntf_triggers_org_event_pattern_idx').on(table.organizationId, table.eventPattern),
    index('ntf_triggers_org_active_idx').on(table.organizationId, table.isActive),
  ],
)
...
export const ntfLogs = pgTable(
  ...
  (table) => [
    ...
    index('ntf_logs_org_template_key_idx').on(table.organizationId, table.templateKey),
  ],
)
