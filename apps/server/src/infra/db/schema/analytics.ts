...
export const anlMetrics = pgTable(
  ...
  (table) => [uniqueIndex('anl_metrics_org_key_idx').on(table.organizationId, table.key)],
)
...
export const anlSnapshots = pgTable(
  ...
  (table) => [
    ...
    index('anl_snapshots_org_metric_idx').on(table.organizationId, table.metricKey),
  ],
)
...
export const anlReportDefinitions = pgTable(
  ...
  (table) => [
    index('anl_report_definitions_org_scheduled_idx').on(table.organizationId, table.isScheduled),
  ],
)
