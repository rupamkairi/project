...
export const itemStatusEnum = pgEnum('cat_item_status', ['draft', 'active', 'archived'])
...
export const catCategories = pgTable(
  ...
  (table) => [
    uniqueIndex('cat_categories_org_slug_idx').on(table.organizationId, table.slug),
    index('cat_categories_org_parent_idx').on(table.organizationId, table.parentId),
    index('cat_categories_org_status_idx').on(table.organizationId, table.status),
  ],
)
...
export const catItems = pgTable(
  ...
  (table) => [
    ...
    index('cat_items_org_category_idx').on(table.organizationId, table.categoryId),
  ],
)
...
export const priceListStatusEnum = pgEnum('cat_price_list_status', ['draft', 'active', 'archived'])
...
export const catPriceRules = pgTable(
  ...
  (table) => [
    ...
    index('cat_price_rules_org_variant_idx').on(table.organizationId, table.variantId),
  ],
)
