...
import type { DomainEvent, OutboxRecord, ReadOptions, EventFilter } from './index'

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function makeEvent(type: string, overrides: Partial<DomainEvent> = {}): DomainEvent {
...
describe('InMemoryEventOutbox — F3: writeBatch', () => {
  test('writeBatch writes all events as separate outbox records', async () => {
    ...
    expect(types).toEqual(['order.confirmed', 'order.placed', 'order.shipped'])
  })
...
describe('InMemoryEventStore — F4: ReadOptions after/from/to', () => {
  ...
  test("'limit' caps the number of results", async () => {
    ...
    for (let v = 1; v <= 5; v++) {
      await store.append(
        createDomainEvent('order.placed', aggId, 'Order', {}, 'org-1' as DomainEvent['orgId'], {
          version: v,
        }),
      )
    }
...
describe('InMemoryEventStore — F5: EventFilter types[] and actorId', () => {
  test('replay filters by types array (matches any of the given types)', async () => {
    ...
    const events = [
      createDomainEvent('order.placed', 'agg-1' as DomainEvent['aggregateId'], 'Order', {}, orgId, {
        version: 1,
      }),
      createDomainEvent(
        'order.confirmed',
        'agg-2' as DomainEvent['aggregateId'],
        'Order',
        {},
        orgId,
        { version: 1 },
      ),
      createDomainEvent('user.created', 'agg-3' as DomainEvent['aggregateId'], 'User', {}, orgId, {
        version: 1,
      }),
    ]
  ...
  test('replay with empty types array matches all events', async () => {
    ...
    await store.append(
      createDomainEvent('user.created', 'agg-2' as DomainEvent['aggregateId'], 'User', {}, orgId, {
        version: 1,
      }),
    )
  ...
  test('replay filters by actorId', async () => {
    ...
    await store.append(
      createDomainEvent('order.placed', 'agg-2' as DomainEvent['aggregateId'], 'Order', {}, orgId, {
        actorId: actorB,
        version: 1,
      }),
    )
