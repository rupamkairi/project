...
function makeManifest(id: string, dependsOn?: string[]): ModuleManifest {
  return {
    id,
    version: '0.1.0',
    dependsOn,
...
describe('createModuleRegistry', () => {
  describe('register + resolve', () => {
    ...
    it('resolve throws NotFoundError for unknown module id', () => {
      ...
      expect(() => registry.resolve('unknown')).toThrow('Module "unknown" is not registered')
    })
  })
  ...
  describe('boot()', () => {
    ...
    it('boots modules in topological (dependency-first) order', async () => {
      ...
      expect(bootOrder.indexOf('identity')).toBeLessThan(bootOrder.indexOf('scheduling'))
      expect(bootOrder.indexOf('catalog')).toBeLessThan(bootOrder.indexOf('inventory'))
      expect(bootOrder.indexOf('catalog')).toBeLessThan(bootOrder.indexOf('scheduling'))
    })
  ...
  describe('shutdown()', () => {
    it('shuts down booted modules in reverse boot order', async () => {
      ...
      expect(shutdownOrder.indexOf('inventory')).toBeLessThan(shutdownOrder.indexOf('catalog'))
    })
  })
