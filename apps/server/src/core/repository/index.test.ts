...
class FakeDb implements DatabaseAdapter {
  ...
  async insert<R>(_table: string, row: Record<string, unknown>): Promise<R> {
    const id = row['id'] as string
    this.rows.set(id, { ...row })
    return { ...row } as R
  }

  async update<R>(_table: string, id: ID, patch: Record<string, unknown>): Promise<R> {
...
class TestRepository extends BaseRepository<TestEntity> {
  buildQuery(filter: Filter): DbQuery {
    ...
    if (filter.field === 'id' && filter.op === 'eq') {
      return {
        sql: 'SELECT * FROM test WHERE org = ? AND id = ?',
        params: [this.orgId, filter.value],
      }
    }
...
describe('Repository — findByIdOrFail', () => {
  test('throws NotFoundError when entity not found', async () => {
    ...
    await expect(repo.findByIdOrFail('nonexistent-id')).rejects.toBeInstanceOf(NotFoundError)
  })
