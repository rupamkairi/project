...
function makeMediatorWithCtx(actorId = 'actor-test') {
  return createMediator({
    contextFactory: (req) => makeMinimalContext((req as Command).actorId ?? actorId),
  })
}
...
describe('createMediator — contextFactory', () => {
  it('passes the factory-built context to the command handler', async () => {
    ...
    const mediator = createMediator({
      contextFactory: (req) => makeMinimalContext((req as Command).actorId ?? 'unknown'),
    })
  ...
  it('passes context through middleware pipeline', async () => {
    ...
    const mediator = createMediator({
      contextFactory: (req) => makeMinimalContext((req as Command).actorId ?? 'mw-actor'),
    })
...
describe('createMediator — no contextFactory', () => {
  it('dispatch throws a clear error when contextFactory is not configured', async () => {
    ...
    await expect(mediator.dispatch(makeCmd({ type: 'no-ctx.cmd' }))).rejects.toThrow(
      'Mediator: contextFactory not configured',
    )
  })
  ...
  it('query throws a clear error when contextFactory is not configured', async () => {
    ...
    await expect(mediator.query(makeQuery({ type: 'no-ctx.query' }))).rejects.toThrow(
      'Mediator: contextFactory not configured',
    )
  })
})
...
describe('AuthorizationMiddleware', () => {
  ...
  it('throws AuthorizationError when check returns false', async () => {
    ...
    await expect(mediator.dispatch(makeCmd({ type: 'auth.cmd' }))).rejects.toThrow(
      AuthorizationError,
    )
  })
...
describe('ValidationMiddleware', () => {
  ...
  it('throws ValidationError when validator returns failures', async () => {
    const mediator = makeMediatorWithCtx()
    mediator.use(ValidationMiddleware(() => [{ field: 'email', message: 'Required' }]))
    mediator.registerCommand('val.cmd', async () => ({ ok: true }))

    await expect(mediator.dispatch(makeCmd({ type: 'val.cmd' }))).rejects.toThrow(ValidationError)
  })
  ...
  it('supports async validators', async () => {
    const mediator = makeMediatorWithCtx()
    mediator.use(ValidationMiddleware(async () => [{ field: 'x', message: 'bad' }]))
    mediator.registerCommand('val.cmd', async () => ({}))
...
describe('IdempotencyMiddleware', () => {
  it('executes the handler on first dispatch', async () => {
    ...
    const result = await mediator.dispatch(makeCmd({ type: 'idem.cmd', idempotencyKey: 'key-1' }))
...
describe('LoggingMiddleware', () => {
  ...
  it('calls logger.error and rethrows on handler failure', async () => {
    ...
    await expect(mediator.dispatch(makeCmd({ type: 'log.fail' }))).rejects.toThrow('handler error')
...
describe('TracingMiddleware', () => {
  ...
  it('attaches correlationId to the span', async () => {
    ...
    await mediator.dispatch(makeCmd({ type: 'trace.cmd', correlationId: 'corr-xyz' }))
  ...
  it('records endedAt after the request completes', async () => {
    ...
    expect(tracingStore[0]!.endedAt).toBeGreaterThanOrEqual(tracingStore[0]!.startedAt)
  })
  ...
  it('records endedAt even when handler throws', async () => {
    ...
    await expect(mediator.dispatch(makeCmd({ type: 'trace.fail' }))).rejects.toThrow('boom')
...
describe('RateLimitMiddleware', () => {
  ...
  it('throws AuthorizationError after limit is exceeded within window', async () => {
    ...
    // Third fails
    await expect(mediator.dispatch(makeCmd({ type: 'rl.cmd' }))).rejects.toThrow(AuthorizationError)
  })
  ...
  it('enforces limits per actor independently', async () => {
    ...
    const result = await mediator.dispatch(makeCmd({ type: 'rl.cmd', actorId: 'actor-B' }))
    expect(result).toEqual({ ok: true })
  })
})
