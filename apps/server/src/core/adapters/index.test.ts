...
const fakePaymentAdapter: PaymentAdapter = {
  async createPaymentSession(_order: PaymentOrder): Promise<PaymentSession> {
    return {
      sessionId: 'sess-001',
      url: 'https://pay.example.com/sess-001',
...
const fakeGeoAdapter: GeoAdapter = {
  ...
  async getDistanceMatrix(_origins: Coordinates[], _destinations: Coordinates[]): Promise<Matrix> {
    return { rows: [] }
  },
}
...
describe('createAdapterRegistry', () => {
  ...
  test('different adapter types are stored independently', () => {
    ...
    expect(registry.get<NotificationAdapter>('notification.email')).toBe(fakeNotificationAdapter)
...
describe('adapter interface type-level checks', () => {
  test('StorageAdapter: fake impl satisfies the interface', async () => {
    const result = await fakeStorageAdapter.upload('key', Buffer.from('x'), {
      mimeType: 'text/plain',
    })
  ...
  test('PaymentAdapter: fake impl satisfies the interface', async () => {
    const session = await fakePaymentAdapter.createPaymentSession({
      amount: fakeMoney,
      currency: 'USD',
      description: 'test',
    })
  ...
  test('GeoAdapter: fake impl satisfies the interface', async () => {
    ...
    const route = await fakeGeoAdapter.getRoute(
      { lat: 52, lng: 13 },
      [{ lat: 53, lng: 14 }],
      'driving',
    )
    ...
    const matrix = await fakeGeoAdapter.getDistanceMatrix(
      [{ lat: 52, lng: 13 }],
      [{ lat: 53, lng: 14 }],
    )
