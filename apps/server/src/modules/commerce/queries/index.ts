...
> = async (query) => {
  const [row] = await db
    ...
    .where(
      and(
        eq(transactions.id, query.params.id),
        eq(transactions.organizationId, query.orgId),
...
export const listTransactionsHandler: QueryHandler<
  ListParams,
...
export const countTransactionsHandler: QueryHandler<{ type?: string }, number> = async (query) => {
  ...
  if (query.params.type)
    conditions.push(eq(transactions.type, query.params.type as Transaction['type']))
  const [c] = await db
    .select({ value: count() })
    .from(transactions)
    .where(and(...conditions))
