...
export const updatePersonHandler: CommandHandler<UpdatePersonPayload, Person> = async (
  ...
  const [row] = await db
    ...
    .where(
      and(eq(persons.id, id), eq(persons.organizationId, command.orgId), isNull(persons.deletedAt)),
    )
    .returning()
...
export const updatePartyHandler: CommandHandler<UpdatePartyPayload, Party> = async (
  ...
  const [row] = await db
    ...
    .set({ ...patch, updatedAt: new Date() })
    .where(
      and(eq(parties.id, id), eq(parties.organizationId, command.orgId), isNull(parties.deletedAt)),
    )
    .returning()
