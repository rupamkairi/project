...
export const getPipelineHandler: QueryHandler<{ id: string }, Pipeline | null> = async (query) => {
  const [row] = await db
    ...
    .where(
      and(
        eq(pipelines.id, query.params.id),
        eq(pipelines.organizationId, query.orgId),
...
export const listPipelinesHandler: QueryHandler<
  ListParams,
  { items: Pipeline[]; total: number }
> = async (query) => {
...
export const getStagesHandler: QueryHandler<{ pipelineId: string }, PipelineStage[]> = async (
  query,
) => {
  return db
    .select()
