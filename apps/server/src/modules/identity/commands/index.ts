...
export const loginHandler: CommandHandler<LoginPayload, LoginResult> = async (command, context) => {
  ...
  const valid = actor.passwordHash ? await Bun.password.verify(password, actor.passwordHash) : false
  ...
  await db.update(actors).set({ lastLoginAt: now, updatedAt: now }).where(eq(actors.id, actor.id))
...
export const logoutHandler: CommandHandler<LogoutPayload, void> = async (command, context) => {
  ...
  await db
    .update(sessions)
    .set({ revokedAt: new Date(), refreshTokenHash: null, refreshExpiresAt: null, updatedAt: new Date() })
...
export const activateHandler: CommandHandler<ActivatePayload, void> = async (command, context) => {
  ...
  const result = await context.fsm.transition('actor:lifecycle', actor.status, 'activate', {
    entity: { ...actor },
    actor: { id: command.actorId, roles: [], orgId: command.orgId },
  })
...
export const suspendActorHandler: CommandHandler<SuspendActorPayload, void> = async (
  ...
  const result = await context.fsm.transition('actor:lifecycle', actor.status, 'suspend', {
    entity: { ...actor },
    actor: { id: command.actorId, roles: [], orgId: command.orgId },
  })
...
export const activateActorHandler: CommandHandler<ActivateActorPayload, void> = async (
  ...
  const result = await context.fsm.transition('actor:lifecycle', actor.status, 'reactivate', {
    entity: { ...actor },
    actor: { id: command.actorId, roles: [], orgId: command.orgId },
  })
