...
import { lmsCohort, lmsCohortMember, lmsWaitlist } from '../db/schema/lms'
...
export function createCohortRoutes(_mediator: Mediator) {
  return new Elysia({ prefix: '' })
    ...
    .get('/instructor/courses/:id/cohorts', async (ctx) => {
      ...
      if (!hasPermission(actor, 'cohort:create')) {
      ...
      const cohorts = await db
        ...
        .where(
          and(
            eq(lmsCohort.itemId, params.id),
            eq(lmsCohort.organizationId, actor.orgId),
            isNull(lmsCohort.deletedAt),
          ),
        )
      ...
      const memberCounts =
        cohortIds.length > 0
          ? await db
              .select({ cohortId: lmsCohortMember.cohortId, count: count() })
              .from(lmsCohortMember)
              .where(
                and(
                  inArray(lmsCohortMember.cohortId, cohortIds),
                  eq(lmsCohortMember.organizationId, actor.orgId),
                ),
              )
    ...
    .patch(
      ...
      async (ctx) => {
        ...
        const existing = await db
          .select()
          .from(lmsCohort)
          .where(and(eq(lmsCohort.id, params.id), eq(lmsCohort.organizationId, actor.orgId)))
          .limit(1)
        if (!existing[0]) {
          set.status = 404
    ...
    .post('/instructor/cohorts/:id/activate', async (ctx) => {
      ...
      if (existing[0].status !== 'scheduled') {
        set.status = 400
        return { error: `Cannot activate cohort in '${existing[0].status}' status` }
      }
    ...
    .post('/instructor/cohorts/:id/complete', async (ctx) => {
      ...
      if (existing[0].status !== 'active') {
        set.status = 400
        return { error: "Cannot complete cohort in '${existing[0].status}' status" }
      }
    ...
    .post('/instructor/cohorts/:id/cancel', async (ctx) => {
      ...
      const members = await db
        ...
        .where(
          and(
            eq(lmsCohortMember.cohortId, params.id),
            eq(lmsCohortMember.organizationId, actor.orgId),
          ),
        )
      ...
      for (const member of members) {
        if (member.transactionId && droppedStageId) {
          await db
            ...
            .where(eq(transactions.id, member.transactionId))
        }
      }
    ...
    .get('/admin/cohorts', async (ctx) => {
      ...
      const cohorts = await db
        .select()
        .from(lmsCohort)
        .where(and(eq(lmsCohort.organizationId, actor.orgId), isNull(lmsCohort.deletedAt)))
    ...
    .get('/instructor/cohorts/:id/sessions', async (ctx) => {
      const actor = getActor(ctx)
      if (!hasPermission(actor, 'session:create')) {
        ;(ctx as any).set.status = 403
        return { error: 'FORBIDDEN' }
      }
      const { params } = ctx as any
      const sessions = await db
        ...
        .where(
          and(
            ...
            eq(activities.entityId, params.id),
            eq(activities.entityType, 'lms.cohort'),
    ...
    .post(
      '/instructor/cohorts/:id/sessions',
      async (ctx) => {
        const actor = getActor(ctx)
        if (!hasPermission(actor, 'session:create')) {
          ;(ctx as any).set.status = 403
    ...
    .patch(
      ...
      async (ctx) => {
        ...
        const existing = await db
          .select()
          .from(activities)
          .where(and(eq(activities.id, params.id), eq(activities.organizationId, actor.orgId)))
          .limit(1)
        if (!existing[0]) {
          set.status = 404
        ...
        await db
          .update(activities)
          .set({
            subject: body.title ?? existing[0].subject,
            body: body.description ?? existing[0].body,
            dueAt: body.scheduledAt ? new Date(body.scheduledAt) : existing[0].dueAt,
            updatedAt: new Date(),
          })
    ...
    .post('/cohorts/:id/waitlist', async (ctx) => {
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(lmsWaitlist.cohortId, params.id),
            eq(lmsWaitlist.personId, personId),
            eq(lmsWaitlist.status, 'waiting'),
          ),
        )
        .limit(1)
      if (existing[0]) {
        set.status = 409
        return { error: 'Already on waitlist' }
      }
      ...
      await db
        ...
        .where(
          and(
            eq(lmsWaitlist.cohortId, params.id),
            eq(lmsWaitlist.personId, personId),
            eq(lmsWaitlist.organizationId, actor.orgId),
            eq(lmsWaitlist.status, 'waiting'),
          ),
        )
    ...
    .get('/learner/sessions', async (ctx) => {
      ...
      const courseIds = enrollments
        .map((e) => e.transaction_lines.itemId)
      ...
      const cohorts = await db
        ...
        .where(
          and(
            inArray(lmsCohort.itemId, courseIds),
            eq(lmsCohort.organizationId, actor.orgId),
            eq(lmsCohort.status, 'active'),
          ),
        )
      ...
      const sessions = await db
        ...
        .where(
          and(
            ...
            eq(activities.type, 'meeting'),
            eq(activities.entityType, 'lms.cohort'),
            inArray(activities.entityId, cohortIds),
            isNull(activities.deletedAt),
          ),
        )
      ...
      return {
        data: sessions.map((s) => {
          ...
          return {
            ...
            scheduledAt: s.dueAt,
            status: s.status,
            meetingUrl: (s.meta as any)?.meetingUrl ?? null,
          }
        }),
      }
    })
