    ...
    new Elysia({ prefix: '' })
      ...
      .get('/lessons/:lessonId/discussions', async (ctx) => {
        ...
        const discussions = await db
          ...
          .where(
            and(
              eq(lmsDiscussion.lessonId, params.lessonId),
              eq(lmsDiscussion.organizationId, actor.orgId),
              isNull(lmsDiscussion.deletedAt),
            ),
          )
        ...
        const people =
          personIds.length > 0
            ? await db
                .select()
                .from(persons)
                .where(eq(persons.organizationId, actor.orgId))
                .then((all) => all.filter((p) => personIds.includes(p.id)))
            : []
      ...
      .post(
        ...
        async (ctx) => {
          ...
          const lesson = await db
            ...
            .where(
              and(
                eq(lmsLesson.id, params.lessonId),
                eq(lmsLesson.organizationId, actor.orgId),
      ...
      .patch('/discussions/:id/resolve', async (ctx) => {
        ...
        if (
          !hasPermission(actor, 'lms-admin') &&
          existing[0].personId !== (await getPersonIdFromActor(actor.id, actor.orgId))
        ) {
          set.status = 403
        ...
        await db
          .update(lmsDiscussion)
          .set({ isResolved: true, updatedAt: new Date() })
          .where(eq(lmsDiscussion.id, params.id))
      ...
      .get('/discussions/:id/replies', async (ctx) => {
        ...
        const replies = await db
          .select()
          .from(lmsDiscussionReply)
          .where(
            and(
              eq(lmsDiscussionReply.discussionId, params.id),
              eq(lmsDiscussionReply.organizationId, actor.orgId),
            ),
          )
        ...
        return {
          data: replies.map((r) => ({
            id: r.id,
            body: r.body,
            authorName: personMap.get(r.personId) ?? 'Unknown',
      ...
      .delete('/replies/:id', async (ctx) => {
        ...
        const existing = await db
          ...
          .where(
            and(
              eq(lmsDiscussionReply.id, params.id),
              eq(lmsDiscussionReply.organizationId, actor.orgId),
            ),
          )
          .limit(1)
        ...
        await db
          .update(lmsDiscussionReply)
          .set({ deletedAt: new Date(), updatedAt: new Date() })
          .where(eq(lmsDiscussionReply.id, params.id))
        return { id: params.id, deleted: true }
      })
  )
}
