...
import { lmsCourseDetail, lmsModule, lmsLesson, lmsCourseReview } from '../db/schema/lms'
    ...
    new Elysia({ prefix: '' })
            ...
            ? await db
                ...
                .where(
                  and(
                    ...
                    inArray(lmsCourseDetail.itemId, itemIds),
                  ),
                )
            : []
      ...
      .get('/courses/:slug', async (ctx) => {
        ...
        const detail = await db
          .select()
          .from(lmsCourseDetail)
          .where(and(eq(lmsCourseDetail.itemId, item[0].id), isNull(lmsCourseDetail.deletedAt)))
          .limit(1)
        ...
        if (detail[0]?.instructorId) {
          const inst = await db
            .select()
            .from(persons)
            .where(eq(persons.id, detail[0].instructorId))
            .limit(1)
          instructorName = inst[0]
            ? `${inst[0].firstName ?? ''} ${inst[0].lastName ?? ''}`.trim()
            : null
        }
        ...
        const modules = await db
          ...
          .where(
            and(
              eq(lmsModule.itemId, item[0].id),
              eq(lmsModule.isPublished, true),
              isNull(lmsModule.deletedAt),
            ),
          )
        ...
        const lessons =
          moduleIds.length > 0
            ? await db
                ...
                .where(
                  and(
                    inArray(lmsLesson.moduleId, moduleIds),
                    eq(lmsLesson.isPublished, true),
                    isNull(lmsLesson.deletedAt),
                  ),
                )
      ...
      .get('/categories', async (ctx) => {
        ...
        return {
          data: cats.map((c) => ({ id: c.id, name: c.name, slug: c.slug, parentId: c.parentId })),
        }
      })
      ...
      .get('/instructor/courses', async (ctx) => {
        ...
        const modCounts =
          itemIds.length > 0
            ? await db
                .select({ itemId: lmsModule.itemId, count: sql<number>`count(*)` })
                .from(lmsModule)
                .where(and(inArray(lmsModule.itemId, itemIds), isNull(lmsModule.deletedAt)))
        ...
        return {
          data: details.map((d) => {
            ...
            return {
              ...
              moduleCount: modCountMap.get(d.itemId) ?? 0,
      ...
      .get('/instructor/courses/:id', async (ctx) => {
        ...
        const detail = await db
          ...
          .where(
            and(
              eq(lmsCourseDetail.itemId, params.id),
              eq(lmsCourseDetail.organizationId, actor.orgId),
              isNull(lmsCourseDetail.deletedAt),
            ),
          )
        ...
        const modules = await db
          .select()
          .from(lmsModule)
          .where(and(eq(lmsModule.itemId, params.id), isNull(lmsModule.deletedAt)))
          .orderBy(asc(lmsModule.position))
        ...
        return {
          ...
          durationHours: detail[0].durationHours,
          language: detail[0].language,
          instructorId: detail[0].instructorId,
          prerequisites: detail[0].prerequisites,
          thumbnailUrl: detail[0].thumbnailUrl,
          completionThreshold: detail[0].completionThreshold,
      ...
      .patch(
        ...
        async (ctx) => {
          ...
          await db
            .update(lmsCourseDetail)
            .set({
              level: level ?? detail[0].level,
              language: language ?? detail[0].language,
      ...
      .post('/instructor/courses/:id/submit-review', async (ctx) => {
        ...
        if (modCount[0].count === 0) {
          set.status = 400
          return { error: 'Course must have at least 1 module' }
        }
      ...
      .get('/admin/courses', async (ctx) => {
        ...
        const reviews =
          itemIds.length > 0
            ? await db
                .select()
                .from(lmsCourseReview)
                .where(
                  and(
                    inArray(lmsCourseReview.itemId, itemIds),
                    eq(lmsCourseReview.status, 'pending'),
                  ),
                )
            : []
      ...
      .post('/admin/courses/:id/approve', async (ctx) => {
        const actor = getActor(ctx)
        if (!hasPermission(actor, 'course:publish')) {
          ;(ctx as any).set.status = 403
          return { error: 'FORBIDDEN' }
        }
      ...
      .post('/admin/courses/:id/archive', async (ctx) => {
        const actor = getActor(ctx)
        if (!hasPermission(actor, 'course:archive')) {
          ;(ctx as any).set.status = 403
          return { error: 'FORBIDDEN' }
        }
