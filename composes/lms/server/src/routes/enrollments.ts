...
import { lmsCourseDetail, lmsCohortMember, lmsCoupon } from '../db/schema/lms'
...
async function getStageId(
  ...
  const pipe = await db
    .select()
    .from(pipelines)
    .where(and(eq(pipelines.organizationId, orgId), eq(pipelines.entityType, entityType)))
    .limit(1)
  if (!pipe[0]) return null
  const stage = await db
    .select()
    .from(pipelineStages)
    .where(and(eq(pipelineStages.pipelineId, pipe[0].id), eq(pipelineStages.name, stageName)))
    .limit(1)
...
async function validateCoupon(code: string, courseId: string, orgId: string) {
  const coupon = await db
    ...
    .where(
      and(
        eq(lmsCoupon.code, code.toUpperCase()),
        eq(lmsCoupon.organizationId, orgId),
        eq(lmsCoupon.isActive, true),
      ),
    )
    .limit(1)
  ...
  if (coupon[0].maxUses && coupon[0].usedCount >= coupon[0].maxUses)
    throw new Error('COUPON_EXHAUSTED')
  const applicableItems = coupon[0].applicableItemIds as string[]
  if (applicableItems?.length > 0 && !applicableItems.includes(courseId))
...
export function createEnrollmentRoutes(_mediator: Mediator) {
  return new Elysia({ prefix: '' })
    ...
    .get('/enrollments', async (ctx) => {
      ...
      const lines =
        txnIds.length > 0
          ? await db
              ...
              .where(inArray(transactionLines.transactionId, txnIds))
          : []
      ...
      const details =
        itemIds.length > 0
          ? await db.select().from(lmsCourseDetail).where(inArray(lmsCourseDetail.itemId, itemIds))
          : []
      ...
      return {
        data: enrollments.map((e) => {
          ...
          return {
            id: e.id,
            courseId: line?.itemId ?? null,
            courseTitle: course ?? null,
            level: detail?.level ?? null,
            stageName: stageMap.get(e.stageId ?? '') ?? null,
    ...
    .get('/enrollments/:id', async (ctx) => {
      ...
      const enrollment = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.id, params.id), eq(transactions.organizationId, actor.orgId)))
        .limit(1)
      if (!enrollment[0]) {
        set.status = 404
      ...
      const course = lines[0]?.itemId
        ? await db.select().from(catItems).where(eq(catItems.id, lines[0].itemId)).limit(1)
        : null
      const stage = enrollment[0].stageId
        ? await db
            .select()
            .from(pipelineStages)
            .where(eq(pipelineStages.id, enrollment[0].stageId))
            .limit(1)
        : null
      return {
        id: enrollment[0].id,
        personId: enrollment[0].personId,
        courseId: lines[0]?.itemId ?? null,
        courseTitle: course?.[0]?.name ?? null,
        stageName: stage?.[0]?.name ?? null,
        totalAmount: enrollment[0].totalAmount,
        totalCurrency: enrollment[0].totalCurrency,
        createdAt: enrollment[0].createdAt,
      }
    })
    ...
    .post(
      ...
      async (ctx) => {
        const actor = getActor(ctx)
        if (!hasPermission(actor, 'enrollment:create')) {
        ...
        if (!personId) {
          set.status = 400
          return { error: 'Learner profile not found. Contact admin.' }
        }
        ...
        if (couponCode) {
          ...
          if (coupon.discountType === 'percentage') {
            finalPrice = Math.max(
              0,
              finalPrice - Math.round(finalPrice * (coupon.discountValue / 100)),
            )
          } else {
        ...
        const result = await db
          .transaction(async (tx) => {
            const existing = await tx
              ...
              .where(
                and(
                  eq(transactions.type, 'order'),
                  eq(transactions.personId, personId),
                  eq(transactionLines.itemId, courseId),
                  isNull(transactions.deletedAt),
                ),
              )
              .limit(1)
            ...
            const targetStageId =
              finalPrice === 0 ? (inProgressStageId ?? enrolledStageId) : enrolledStageId
            ...
            await tx.insert(transactionLines).values({
              id: `txl_${generateId().slice(0, 20)}`,
              organizationId: actor.orgId,
              transactionId: txnId,
              itemId: courseId,
              qty: 1,
              unitPriceAmount: finalPrice,
              unitPriceCurrency: currency,
              lineTotalAmount: finalPrice,
          ...
          .catch((err: Error) => {
            ...
            if (err.message?.startsWith('COUPON_')) {
              set.status = 400
        ...
        return {
          id: (result as any).id,
          status: (result as any).free ? 'active' : 'pending-payment',
          isFree: (result as any).free,
        }
      },
      {
        body: t.Object({
          courseId: t.String({ minLength: 1 }),
          cohortId: t.Optional(t.String()),
    ...
    .post('/enrollments/:id/cancel', async (ctx) => {
      ...
      if (droppedStageId)
        await db
          .update(transactions)
          .set({ stageId: droppedStageId, updatedAt: new Date() })
    ...
    .get('/admin/enrollments', async (ctx) => {
      const actor = getActor(ctx)
      if (!hasPermission(actor, 'enrollment:manage')) {
        ;(ctx as any).set.status = 403
        return { error: 'FORBIDDEN' }
      }
      ...
      const enrollments = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.organizationId, actor.orgId), eq(transactions.type, 'order')))
        .orderBy(desc(transactions.createdAt))
        .limit(limit)
        .offset(offset)
      ...
      return {
        data: enrollments.map((e) => {
          ...
          return {
            id: e.id,
            learnerName: personMap.get(e.personId ?? '') ?? null,
            courseTitle: line?.itemId ? (courseMap.get(line.itemId) ?? null) : null,
            totalAmount: e.totalAmount,
            stageId: e.stageId,
            createdAt: e.createdAt,
          }
    ...
    .post(
      ...
      async (ctx) => {
        ...
        if (learnerIds.length > 500) {
          set.status = 400
          return { error: 'Max 500 learners per bulk enrollment' }
        }
          ...
          try {
            const existing = await db
              ...
              .where(
                and(
                  eq(transactions.type, 'order'),
                  eq(transactions.personId, learnerId),
                  eq(transactionLines.itemId, courseId),
                  isNull(transactions.deletedAt),
                ),
              )
            ...
            await db.insert(transactions).values({
              ...
              totalAmount: 0,
              totalCurrency: 'USD',
              meta: { bulkEnrollment: true, cohortId },
              createdAt: new Date(),
              updatedAt: new Date(),
              version: 1,
            })
            ...
            errors.push({ learnerId, reason: err.message })
          }
        }
      ...
      },
      {
        body: t.Object({
          courseId: t.String({ minLength: 1 }),
          learnerIds: t.Array(t.String(), { minItems: 1, maxItems: 500 }),
          cohortId: t.Optional(t.String()),
        }),
      },
    )
