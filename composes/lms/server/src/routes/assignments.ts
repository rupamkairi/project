    ...
    new Elysia({ prefix: '' })
      ...
      .get('/instructor/courses/:id/assignments', async (ctx) => {
        ...
        const modules = await db
          .select()
          .from(lmsModule)
          .where(and(eq(lmsModule.itemId, params.id), eq(lmsModule.organizationId, actor.orgId)))
        ...
        const assignments = await db
          ...
          .where(
            and(
              inArray(lmsAssignment.moduleId, moduleIds),
              eq(lmsAssignment.organizationId, actor.orgId),
              isNull(lmsAssignment.deletedAt),
            ),
          )
      ...
      .post(
        ...
        async (ctx) => {
          ...
          const mod = await db
            ...
            .where(
              and(
                eq(lmsModule.id, moduleId),
                eq(lmsModule.itemId, params.id),
      ...
      .get('/assignments/:id', async (ctx) => {
        ...
        const assignment = await db
          .select()
          .from(lmsAssignment)
          .where(and(eq(lmsAssignment.id, params.id), isNull(lmsAssignment.deletedAt)))
          .limit(1)
        ...
        const submissions = personId
          ? await db
              ...
              .where(
                and(
                  eq(lmsSubmission.assignmentId, params.id),
                  eq(lmsSubmission.personId, personId),
                ),
              )
      ...
      .post(
        ...
        async (ctx) => {
          ...
          const mod = await db
            .select()
            .from(lmsModule)
            .where(and(eq(lmsModule.id, assignment[0].moduleId), isNull(lmsModule.deletedAt)))
            .limit(1)
      ...
      .get('/instructor/assignments/:id/submissions', async (ctx) => {
        ...
        const submissions = await db
          ...
          .where(
            and(
              eq(lmsSubmission.assignmentId, params.id),
              eq(lmsSubmission.organizationId, actor.orgId),
            ),
          )
          .orderBy(desc(lmsSubmission.submittedAt))
