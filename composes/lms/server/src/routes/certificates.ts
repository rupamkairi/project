...
import { lmsCertificate, lmsCourseDetail } from '../db/schema/lms'
    ...
    new Elysia({ prefix: '' })
      ...
      .post('/certificates/issue/:transactionId', async (ctx) => {
        ...
        const line = await db
          ...
          .where(eq(transactionLines.transactionId, params.transactionId))
          .limit(1)
        ...
        const detail = await db
          ...
          .where(
            and(
              eq(lmsCourseDetail.itemId, itemId),
        ...
        const existing = await db
          .select()
          .from(lmsCertificate)
          .where(eq(lmsCertificate.transactionId, params.transactionId))
          .limit(1)
        ...
        while (retries < 5) {
          const dup = await db
            .select()
            .from(lmsCertificate)
            .where(eq(lmsCertificate.certificateNo, certificateNo))
            .limit(1)
          if (!dup[0]) break
      ...
      .get('/certificates/:id', async (ctx) => {
        ...
        const person = await db
          .select()
          .from(persons)
          .where(eq(persons.id, cert[0].personId))
          .limit(1)
        const course = await db
          .select()
          .from(catItems)
          .where(eq(catItems.id, cert[0].itemId))
          .limit(1)
        ...
        return {
          ...
          learnerName: person[0]
            ? `${person[0].firstName ?? ''} ${person[0].lastName ?? ''}`.trim()
            : null,
      ...
      .get('/verify/:code', async (ctx) => {
        ...
        const cert = await db
          .select()
          .from(lmsCertificate)
          .where(eq(lmsCertificate.verificationCode, code))
          .limit(1)
