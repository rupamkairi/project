...
export function createAnalyticsRoutes(_mediator: Mediator) {
  return new Elysia({ prefix: '' })
    ...
    .get('/admin/analytics/overview', async (ctx) => {
      ...
      const avgOrderValue =
        allEnrollments.length > 0 ? Math.round(totalRevenue / allEnrollments.length) : 0

      // Top courses by revenue
      const lineItems = await db
        .select()
        .from(transactionLines)
        .where(eq(transactionLines.organizationId, actor.orgId))
      ...
      const topCourses = Array.from(courseRevenueMap.entries())
        ...
        .map(([courseId, data]) => ({
          courseId,
          title: courseNameMap.get(courseId) ?? null,
      ...
      return {
        enrollments: {
          ...
          cancelled: dropped.length,
          periodNew: periodEnrollments.length,
          completionRate,
        },
        courses: {
          total: courseDetails.length,
          published: published.length,
          underReview: courseDetails.length - published.length,
        },
    ...
    .get('/admin/analytics/revenue', async (ctx) => {
      ...
      return {
        ...
        freeEnrollments: enrollments.length - paidCount,
        totalEnrollments: enrollments.length,
        avgRevenuePerPaid: paidCount > 0 ? Math.round(totalRevenue / paidCount) : 0,
      }
    })
    ...
    .get('/instructor/analytics/courses/:id', async (ctx) => {
      ...
      const activeCount = enrollments.filter(
        (e) => e.transactions.stageId !== completedStageId,
      ).length
      const completedCount = enrollments.filter(
        (e) => e.transactions.stageId === completedStageId,
      ).length
      const totalRevenue = enrollments.reduce(
        (sum, e) => sum + (e.transactions.totalAmount ?? 0),
        0,
      )
      ...
      const modules = await db
        .select()
        .from(lmsModule)
        .where(and(eq(lmsModule.itemId, itemId), eq(lmsModule.organizationId, actor.orgId)))
        .orderBy(lmsModule.position)

      const moduleIds = modules.map((m) => m.id)
      const personIds = enrollments.map((e) => e.transactions.personId).filter(Boolean) as string[]
      const allProgress =
        personIds.length > 0 && itemId
          ? await db
              .select()
              .from(lmsProgress)
              .where(and(inArray(lmsProgress.personId, personIds), eq(lmsProgress.itemId, itemId)))
          : []
        ...
        return {
          ...
          completionRate:
            modProgress.length > 0
              ? Math.round((completedMod.length / modProgress.length) * 100)
              : 0,
        }
      })
      ...
      return {
        course: {
          id: itemId,
          title: item[0]?.name ?? '',
          isPublished: detail[0].isPublished,
          publishedAt: detail[0].publishedAt,
        },
        enrollments: {
    ...
    .get('/instructor/analytics/overview', async (ctx) => {
      ...
      const details = await db
        ...
        .where(
          and(
            eq(lmsCourseDetail.organizationId, actor.orgId),
            eq(lmsCourseDetail.instructorId, actor.id),
            isNull(lmsCourseDetail.deletedAt),
          ),
        )
      ...
      const lineItems =
        itemIds.length > 0
          ? await db
              .select()
              .from(transactionLines)
              .where(inArray(transactionLines.itemId, itemIds))
          : []
    ...
    .get('/learner/analytics', async (ctx) => {
      ...
      const allProgress = await db
        .select()
        .from(lmsProgress)
        .where(and(eq(lmsProgress.personId, personId), eq(lmsProgress.organizationId, actor.orgId)))
      ...
      const [certificateCount] = await db
        .select({ count: count() })
        .from(lmsCertificate)
        .where(and(eq(lmsCertificate.personId, personId), eq(lmsCertificate.isRevoked, false)))
      ...
      const courseIds = enrollments
        .map((e) => lineItems.find((l) => l.transactionId === e.id)?.itemId)
        .filter(Boolean) as string[]
      const courses =
        courseIds.length > 0
          ? await db.select().from(catItems).where(inArray(catItems.id, courseIds))
          : []
      const courseNameMap = new Map(courses.map((c) => [c.id, c.name]))
      ...
      const modules =
        allCourseIds.length > 0
          ? await db.select().from(lmsModule).where(inArray(lmsModule.itemId, allCourseIds))
          : []
      const moduleIds = modules.map((m) => m.id)
      const allLessons =
        moduleIds.length > 0
          ? await db.select().from(lmsLesson).where(inArray(lmsLesson.moduleId, moduleIds))
          : []
      ...
      const courseSummaries = courseIds.map((courseId) => {
        ...
        const lastAccessed =
          personProgress.length > 0
            ? personProgress.reduce(
                (latest, p) => (p.updatedAt > latest ? p.updatedAt : latest),
                personProgress[0].updatedAt,
              )
            : null
        return {
          courseId,
          title: courseNameMap.get(courseId) ?? null,
          completionPct: Math.round((completeCount / courseLessonCount) * 100),
          lastAccessedAt: lastAccessed,
          timeSpentMinutes: Math.round(
            personProgress.reduce((sum, p) => sum + (p.watchedSeconds ?? 0), 0) / 60,
          ),
        }
      })

      return {
        enrollments: { total: enrollments.length, active: activeCount, completed: completedCount },
        totalHoursSpent: Math.round(totalWatchSeconds / 3600),
