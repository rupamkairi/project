    ...
    new Elysia({ prefix: '' })
        ...
        const progress = await db
          ...
          .where(
            and(
              ...
              eq(lmsProgress.itemId, params.courseId),
              eq(lmsProgress.organizationId, actor.orgId),
            ),
          )
          ...
          const existing = await db
            ...
            .where(
              and(
                eq(lmsProgress.organizationId, actor.orgId),
                eq(lmsProgress.personId, personId),
                eq(lmsProgress.lessonId, lessonId),
              ),
            )
            .limit(1)
      ...
      .get('/unlock/:courseId/modules', async (ctx) => {
        ...
        const modules = await db
          ...
          .where(
            and(
              eq(lmsModule.itemId, params.courseId),
              eq(lmsModule.organizationId, actor.orgId),
        ...
        const completedModuleIds = new Set(
          progress.filter((p) => p.isCompleted).map((p) => p.moduleId),
        )
      ...
      .post(
        ...
        async (ctx) => {
          ...
          if (attempts[0].count >= (orgConfig?.maxQuizAttempts ?? 3)) {
            set.status = 403
            return {
              error: 'MAX_QUIZ_ATTEMPTS_REACHED',
              maxAttempts: orgConfig?.maxQuizAttempts ?? 3,
            }
          }
          ...
          const quiz = await db
            ...
            .where(
              and(
                eq(lmsQuiz.id, quizId),
                eq(lmsQuiz.lessonId, lessonId),
                eq(lmsQuiz.organizationId, actor.orgId),
              ),
            )
            .limit(1)
          ...
          const questions = await db
            .select()
            .from(lmsQuizQuestion)
            .where(
              and(
                eq(lmsQuizQuestion.quizId, quizId),
                eq(lmsQuizQuestion.organizationId, actor.orgId),
              ),
            )
          ...
          for (const question of questions) {
            ...
            if (question.type === 'true_false' || question.type === 'mcq') {
              ...
              isCorrect =
                answer.text?.toLowerCase().trim() === question.correctAnswer?.toLowerCase().trim()
            }
          ...
          if (passed) {
            ...
            if (existing[0]) {
              await db
                .update(lmsProgress)
                .set({ isCompleted: true, score: percentage, updatedAt: new Date() })
                .where(eq(lmsProgress.id, existing[0].id))
            } else {
          ...
          moduleIds.length > 0
            ? ((
                await db
                  .select({ count: count() })
                  .from(lmsLesson)
                  .where(inArray(lmsLesson.moduleId, moduleIds))
              )[0]?.count ?? 1)
            : 1
