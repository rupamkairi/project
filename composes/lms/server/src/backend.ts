    ...
    setTimeout(async () => {
      try {
        await db
          ...
          .where(and(eq(lmsProgress.personId, personId), eq(lmsProgress.lessonId, lessonId)))
      } catch (err) {
...
export async function getOrgConfig(orgId: string) {
  ...
  const config = await db
    .select()
    .from(lmsOrgConfig)
    .where(eq(lmsOrgConfig.organizationId, orgId))
    .limit(1)
...
export function registerLmsHooks(bus: EventBus, mediator: Mediator) {
  ...
  bus.subscribe('lms.lesson.completed', async (event: any) => {
    ...
    const allLessons = await db
      .select({ id: lmsLesson.id })
      .from(lmsLesson)
      .innerJoin(lmsModule, eq(lmsModule.id, lmsLesson.moduleId))
      .where(
        and(
          eq(lmsModule.itemId, itemId),
          eq(lmsLesson.organizationId, orgId),
    ...
    const detail = await db
      ...
      .where(and(eq(lmsCourseDetail.itemId, itemId), eq(lmsCourseDetail.organizationId, orgId)))
      .limit(1)

    const pct = Math.round((completed.count / allLessonIds.length) * 100)
    if (pct >= (detail[0]?.completionThreshold ?? 80)) {
      const enrollmentTxn = await db
        .select()
        .from(transactions)
      ...
      if (enrollmentTxn[0]) {
        ...
        if (completedStageId) {
          await db
            .update(transactions)
            .set({ stageId: completedStageId, updatedAt: new Date() })
        ...
        bus.publish(
          makeEvent(
            ...
            'lms.enrollment',
            {
              personId,
              itemId,
              transactionId: enrollmentTxn[0].transactions.id,
              orgId,
              percentage: pct,
            },
            'system',
  ...
  bus.subscribe('lms.enrollment.completed', async (event: any) => {
    ...
    for (let i = 0; i < 8; i++)
      certificateNo += chars.charAt(Math.floor(Math.random() * chars.length))
  ...
  bus.subscribe('lms.cohort.cancelled', async (event: any) => {
    ...
    const members = await db
      ...
      .where(eq(lmsCohortMember.cohortId, cohortId))
  ...
  bus.subscribe('payment.succeeded', async (event: any) => {
    ...
    if (metadata?.transactionId) {
      ...
      if (inProgressStageId) {
        await db
          .update(transactions)
          .set({ stageId: inProgressStageId, updatedAt: new Date() })
  ...
  bus.subscribe('lms.assignment.graded', async (event: any) => {
    ...
    await mediator
      .dispatch({
        ...
        payload: {
          personId,
          template: 'lms.assignment.graded',
          data: { assignmentTitle, score, maxScore, feedback },
        },
        actorId: 'system',
        orgId,
        correlationId: generateId(),
      ...
      if (session[0]) {
        await mediator
          .dispatch({
            ...
            payload: {
              entityId: session[0].entityId,
              entityType: 'lms.cohort',
              template: 'lms.live.session.reminder',
              data: { sessionTitle: session[0].subject, sessionStart: session[0].dueAt },
            },
            actorId: 'system',
            orgId,
            correlationId: generateId(),
    ...
    async () => {
      ...
      for (const config of configs) {
        ...
        const inactiveTxns = await db
          ...
          .leftJoin(
            lmsProgress,
            and(eq(lmsProgress.transactionId, transactions.id), gt(lmsProgress.updatedAt, cutoff)),
          )
          .where(
            and(
        ...
        mediator
          .dispatch({
            ...
            payload: {
              ...
              entityType: 'lms.cohort',
              template: 'lms.live.session.reminder',
              data: { sessionTitle: session.subject, sessionStart: session.dueAt },
            },
            actorId: 'system',
