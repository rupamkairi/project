...
import { ntfTemplates, ntfTriggers, ntfLogs, ntfPreferences } from '@db/schema/notification'
  ...
  return (
    new Elysia({ prefix: '/notifications' })
      ...
      .get('/templates', async ({ query, headers, set }) => {
        ...
        const conditions = [eq(ntfTemplates.organizationId, orgId), isNull(ntfTemplates.deletedAt)]
      ...
      .get('/triggers', async ({ query, headers, set }) => {
        ...
        const [triggers, countResult] = await Promise.all([
          db
            .select()
            .from(ntfTriggers)
            .where(and(eq(ntfTriggers.organizationId, orgId), isNull(ntfTriggers.deletedAt)))
      ...
      .post('/triggers', async ({ body, headers, set }) => {
        ...
        const { eventPattern, templateKey, channel, recipientExpr, conditions } = body as {
          eventPattern: string
          templateKey: string
          channel: string
      ...
      .delete('/triggers/:id', async ({ params, headers, set }) => {
        ...
        await db
          .update(ntfTriggers)
          .set({ deletedAt: new Date() })
          .where(and(eq(ntfTriggers.id, params.id), eq(ntfTriggers.organizationId, orgId)))
