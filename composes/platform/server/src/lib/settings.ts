...
export async function getSetting(key: string): Promise<PlatformSetting | undefined> {
  const [setting] = await db.select().from(pltSettings).where(eq(pltSettings.key, key)).limit(1)
...
export async function getPublicSettings(): Promise<PlatformSetting[]> {
  const settings = await db.select().from(pltSettings).where(eq(pltSettings.isPublic, true))
  return settings as PlatformSetting[]
}
