...
import { erpAsset, erpAssetDepreciation } from '../db/schema/erp'
    ...
    for (const asset of assets) {
      ...
      if (dep) {
        const newBook = Math.max(
          0,
          Number(asset.bookValue ?? asset.purchaseValue) - monthlyDepreciation,
        )
        await db
          .update(erpAsset)
          .set({
