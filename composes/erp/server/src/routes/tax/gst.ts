  ...
  return new Elysia()
    .get('/gst-templates', async (ctx) => {
      ...
      const rows = await db
        .select()
        .from(erpGstTemplate)
        .where(eq(erpGstTemplate.organizationId, actor.orgId))
    ...
    .get('/gst-returns/gstr1/preview', async (ctx) => {
      ...
      for (const inv of outbound) {
        ...
        const lines = await db
          ...
          .where(eq(transactionLines.transactionId, inv.id))
        const gst = {
          cgst: Number(invMeta?.cgst ?? 0),
          sgst: Number(invMeta?.sgst ?? 0),
          igst: Number(invMeta?.igst ?? 0),
        }
    ...
    .get('/gst-returns/gstr3b/preview', async (ctx) => {
      ...
      const outbound = invoices.filter(
        (i: any) => (i.meta as any)?.direction === 'outbound' && i.status === 'submitted',
      )
      const inbound = invoices.filter(
        (i: any) => (i.meta as any)?.direction === 'inbound' && i.status !== 'draft',
      )
