  ...
  return new Elysia({ prefix: '/purchase-orders' })
    ...
    .post('/', async (ctx) => {
      ...
      const [vendor] = await db
        .select()
        .from(parties)
        .where(and(eq(parties.id, body.vendorId), eq(parties.type, 'vendor')))
    ...
    .post('/:id/submit', async (ctx) => {
      ...
      await db
        ...
        .where(and(eq(transactions.id, id), eq(transactions.type, 'purchase_order')))
    ...
    .post('/:id/reject', async (ctx) => {
      ...
      await db
        .update(transactions)
        .set({ status: 'rejected', meta: { rejectedReason: body.reason } } as any)
        .where(eq(transactions.id, id))
