  ...
  return new Elysia({ prefix: '/goods-receipts' })
    ...
    .post('/', async (ctx) => {
      ...
      const poLines = await db
        ...
        .where(eq(transactionLines.transactionId, body.transactionId))
      for (const grnItem of body.items) {
        ...
        const alreadyReceived = existingGrns.reduce(
          (s: number, g: any) => s + Number(g.qtyReceived),
          0,
        )
        ...
        if (grnItem.receivedQty > remaining) {
          ;(ctx as any).set.status = 400
          return {
            error: `Cannot receive more than ordered for item ${grnItem.itemId}. Remaining: ${remaining}`,
          }
        }
      }
    ...
    .get('/:id', async (ctx) => {
      ...
      const [grn] = await db
        .select()
        .from(erpGrn)
        .where(and(eq(erpGrn.id, id), eq(erpGrn.organizationId, actor.orgId)))
      if (!grn) {
        ;(ctx as any).set.status = 404
    ...
    .post('/:id/confirm', async (ctx) => {
      ...
      await db.transaction(async (tx) => {
        ...
        const totalValue = grnItems.reduce(
          (s: number, i: any) => s + Number(i.qtyAccepted) * Number(i.valuationRate ?? 0),
          0,
        )
