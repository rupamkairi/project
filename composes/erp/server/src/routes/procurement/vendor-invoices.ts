...
export function createVendorInvoiceRoutes(mediator: Mediator) {
  return new Elysia({ prefix: '/vendor-invoices' })
    .get('/', async (ctx) => {
      ...
      const vendorInvoices = rows.filter(
        (r: any) =>
          (r.meta as any)?.direction === 'inbound' || (r.meta as any)?.subType === 'vendor_invoice',
      )
    ...
    .get('/:id', async (ctx) => {
      ...
      const [inv] = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.id, id), eq(transactions.organizationId, actor.orgId)))
      if (!inv) {
    ...
    .post('/:id/dispute', async (ctx) => {
      ...
      await db
        .update(transactions)
        .set({ status: 'disputed', meta: { disputeReason: body.reason } } as any)
        .where(eq(transactions.id, id))
    ...
    .post('/:id/match', async (ctx) => {
      ...
      if (poId) {
        ...
        if (po) {
          ...
          checks.priceMatch =
            poTotal === 0 || Math.abs(invoiceTotal - poTotal) / poTotal <= INVOICE_TOLERANCE_PCT
      ...
      if (allPassed) {
        await db
          .update(transactions)
          .set({ status: '3way-matched' } as any)
          .where(eq(transactions.id, id))
      }
