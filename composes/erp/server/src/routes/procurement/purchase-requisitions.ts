...
import { erpPurchaseRequisition, erpPrItem } from '../../db/schema/erp'
  ...
  return new Elysia({ prefix: '/purchase-requisitions' })
    ...
    .post('/', async (ctx) => {
      ...
      const refNo = await nextRefNo(
        ...
        erpPurchaseRequisition.refNo,
      )

      const [pr] = await db
        .insert(erpPurchaseRequisition)
        .values({
    ...
    .get('/:id', async (ctx) => {
      ...
      const [pr] = await db
        ...
        .where(
          and(
            eq(erpPurchaseRequisition.id, id),
            eq(erpPurchaseRequisition.organizationId, actor.orgId),
          ),
        )
      if (!pr) {
