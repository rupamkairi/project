  ...
  return new Elysia({ prefix: '/vendors' })
    .get('/', async (ctx) => {
      ...
      const rows = await db
        .select()
        .from(parties)
        .where(and(eq(parties.type, 'vendor'), eq(parties.organizationId, orgId)))
      return { vendors: rows }
    })
    ...
    .get('/:id', async (ctx) => {
      ...
      const [vendor] = await db
        .select()
        .from(parties)
        .where(and(eq(parties.id, id), eq(parties.type, 'vendor')))
      if (!vendor) {
    ...
    .post('/:id/blacklist', async (ctx) => {
      ...
      await db
        .update(parties)
        .set({ status: 'blacklisted', meta: { blacklistReason: body.reason } } as any)
        .where(and(eq(parties.id, id), eq(parties.type, 'vendor')))
