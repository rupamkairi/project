  ...
  return new Elysia({ prefix: '/finance' })
    ...
    .get('/ap-aging', async (ctx) => {
      ...
      const vendorInvoices = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.type, 'invoice'), eq(transactions.organizationId, actor.orgId)))

      const inbound = vendorInvoices.filter(
        (i: any) => (i.meta as any)?.direction === 'inbound' && i.status !== 'paid',
      )
    ...
    .get('/ar-aging', async (ctx) => {
      ...
      const outbound = salesInvoices.filter(
        (i: any) => (i.meta as any)?.direction === 'outbound' && i.status !== 'paid',
      )
      const aging: Record<string, any> = {}
    ...
    .get('/bank-accounts', async (ctx) => {
      ...
      const rows = await db
        .select()
        .from(erpBankAccount)
        .where(eq(erpBankAccount.organizationId, actor.orgId))
    ...
    .get('/bank-reconciliation/:bankAccountId', async (ctx) => {
      ...
      const unmatched = await db
        ...
        .where(
          and(
            eq(erpBankTransaction.bankAccountId, bankAccountId),
            eq(erpBankTransaction.status, 'unmatched'),
          ),
        )
    ...
    .get('/fiscal-years', async (ctx) => {
      ...
      const rows = await db
        .select()
        .from(erpFiscalYear)
        .where(eq(erpFiscalYear.organizationId, actor.orgId))
    ...
    .post('/period-close', async (ctx) => {
      ...
      if (draftJEs.length > 0) {
        ;(ctx as any).set.status = 400
        return {
          error: `${draftJEs.length} draft journal entries must be posted or cancelled before period close`,
        }
      }

      await db
        .update(erpFiscalYear)
        .set({ isClosed: true })
        .where(eq(erpFiscalYear.id, fiscalYearId))
