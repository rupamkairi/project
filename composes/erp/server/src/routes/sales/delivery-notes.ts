...
export function createDeliveryNoteRoutes(mediator: Mediator) {
  return new Elysia({ prefix: '/delivery-notes' })
    ...
    .post('/', async (ctx) => {
      ...
      const dnNumber = await nextRefNo(
        db,
        orgId,
        'DN',
        year,
        erpDeliveryNote,
        erpDeliveryNote.dnNumber,
      )

      const [dn] = await db
        .insert(erpDeliveryNote)
        .values({
      ...
      const soLines = await db
        ...
        .where(eq(transactionLines.transactionId, body.soId))

      await db.insert(erpDnItem).values(
        body.items.map((item: any) => {
          const soLine = soLines.find(
            (l: any) => l.id === item.soItemId || l.itemId === item.itemId,
          )
          return {
            dnId: dn.id,
    ...
    .get('/:id', async (ctx) => {
      ...
      const [dn] = await db
        .select()
        .from(erpDeliveryNote)
        .where(and(eq(erpDeliveryNote.id, id), eq(erpDeliveryNote.organizationId, actor.orgId)))
      if (!dn) {
