  ...
  return new Elysia({ prefix: '/sales-orders' })
      ...
      const [so] = await db
        ...
        .where(
          and(
            ...
            eq(transactions.type, 'sales_order'),
    ...
    .post('/:id/confirm', async (ctx) => {
      ...
      const [so] = await db
        .select()
        .from(transactions)
        .where(and(eq(transactions.id, id), eq(transactions.type, 'sales_order')))
      ...
      if (customer) {
        ...
        if (creditLimit > 0 && outstanding + soTotal > creditLimit) {
          ;(ctx as any).set.status = 400
          return {
            error: `Credit limit exceeded. Outstanding: ${outstanding}, SO: ${soTotal}, Limit: ${creditLimit}`,
          }
        }
      }
    ...
    .post('/:id/cancel', async (ctx) => {
      ...
      const dns = await db
        ...
        .where(eq(erpDeliveryNote.transactionId, id))
      if (dns.length > 0) {
        ;(ctx as any).set.status = 400
