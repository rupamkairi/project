  ...
  return new Elysia({ prefix: '/customers' })
    .get('/', async (ctx) => {
      ...
      const rows = await db
        .select()
        .from(parties)
        .where(and(eq(parties.type, 'customer'), eq(parties.organizationId, actor.orgId)))
    ...
    .get('/:id', async (ctx) => {
      ...
      const [customer] = await db
        .select()
        .from(parties)
        .where(and(eq(parties.id, id), eq(parties.type, 'customer')))
      if (!customer) {
    ...
    .patch('/:id', async (ctx) => {
      ...
      await db
        .update(parties)
        .set({ name: body.name ?? existing.name, meta: newMeta })
