...
async function explodeBom(bomId: string, multiplier: number, depth: number): Promise<any[]> {
  ...
  for (const item of items) {
    ...
    const [subBom] = await db
      .select()
      .from(erpBom)
      .where(and(eq(erpBom.itemId, item.componentItemId), eq(erpBom.isActive, true)))
    if (subBom) {
      const subItems = await explodeBom(
        subBom.id,
        required / Number(subBom.quantity ?? 1),
        depth + 1,
      )
  ...
  return new Elysia({ prefix: '/boms' })
    ...
    .post('/', async (ctx) => {
      ...
      const existingBoms = await db
        .select()
        .from(erpBom)
        .where(and(eq(erpBom.itemId, body.itemId), eq(erpBom.organizationId, orgId)))
    ...
    .get('/by-item/:itemId', async (ctx) => {
      ...
      const boms = await db
        .select()
        .from(erpBom)
        .where(and(eq(erpBom.itemId, itemId), eq(erpBom.organizationId, actor.orgId)))
      return { boms }
    })
    ...
    .get('/:id', async (ctx) => {
      ...
      const [bom] = await db
        .select()
        .from(erpBom)
        .where(and(eq(erpBom.id, id), eq(erpBom.organizationId, actor.orgId)))
      if (!bom) {
    ...
    .post('/:id/activate', async (ctx) => {
      ...
      await db
        .update(erpBom)
        .set({ isActive: false })
        .where(and(eq(erpBom.itemId, bom.itemId), eq(erpBom.organizationId, bom.organizationId)))
