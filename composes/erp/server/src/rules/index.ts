...
export function isThreeWayMatchValid(
  ...
  if (invoiceAmount > maxAmount) {
    return {
      valid: false,
      reason: `Invoice ${invoiceAmount} exceeds PO ${poAmount} by more than 5%`,
    }
  }
...
export function isOverReceived(
  orderedQty: number,
  receivedQty: number,
...
export function canPostJournalEntry(isFiscalYearClosed: boolean): {
  allowed: boolean
  reason?: string
} {
...
export function isDoubleEntryBalanced(
