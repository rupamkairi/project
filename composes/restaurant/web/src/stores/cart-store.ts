...
interface CartState {
  ...
  orderType: 'dine-in' | 'takeaway' | 'pickup' | 'partner-delivery'
  tableId: string | null
  specialInstructions: string
  addItem: (
    item: { id: string; name: string; meta?: { basePrice?: string } },
    modifiers?: CartModifier[],
    note?: string | undefined,
  ) => void
  ...
  setOrderType: (t: 'dine-in' | 'takeaway' | 'pickup' | 'partner-delivery') => void
...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  total: () => {
    ...
    const subtotal = items.reduce(
      (s, i) => s + i.qty * (i.unitPrice + i.modifiers.reduce((ms, m) => ms + m.price, 0)),
      0,
    )
    const taxRate = 0.05
