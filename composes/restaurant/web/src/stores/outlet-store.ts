...
export const useOutletStore = create<OutletState>((set) => ({
  outletId:
    typeof localStorage !== 'undefined' ? (localStorage.getItem('rst_outlet_id') ?? null) : null,
  outletName:
    typeof localStorage !== 'undefined' ? (localStorage.getItem('rst_outlet_name') ?? null) : null,
  setOutlet: (id, name) => {
