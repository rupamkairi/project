...
function MenuItemCard({ item, onAdd }: { item: MenuItem; onAdd: () => void }) {
  ...
  return (
    ...
    >
      {item.meta?.isPopular && (
        <Badge variant="secondary" className="mb-1 text-xs bg-amber-100 text-amber-700">
          Popular
        </Badge>
      )}
    ...
    <aside className="w-80 border-l flex flex-col">
      ...
      <div className="border-t p-4 space-y-3">
        <div className="flex justify-between text-sm">
          <span>Subtotal</span>
          <AmountDisplay amount={t.subtotal} />
        </div>
        <div className="flex justify-between text-sm text-muted-foreground">
          <span>Tax (5%)</span>
          <AmountDisplay amount={t.tax} />
        </div>
        <div className="flex justify-between font-bold">
          <span>Total</span>
          <AmountDisplay amount={t.total} />
        </div>
        <Button className="w-full" disabled={items.length === 0 || loading} onClick={onPlace}>
...
export function NewOrderPage() {
  ...
  const [orderType, setOrderType] = useState<
    'dine-in' | 'takeaway' | 'pickup' | 'partner-delivery'
  >(search?.type ?? 'dine-in')
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex gap-2 p-3 border-b">
          {(['dine-in', 'takeaway', 'pickup', 'partner-delivery'] as const).map((t) => (
            <Button
              key={t}
              size="sm"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {visibleItems.map((item) => (
              <MenuItemCard key={item.id} item={item} onAdd={() => addItem(item as any)} />
            ))}
          </div>
        </div>
      </main>
