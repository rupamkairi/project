    ...
    >
      <div className="flex items-center justify-between">
        <p className="font-mono font-bold text-sm">
          {order.meta?.orderNumber ?? order.id.slice(0, 8)}
        </p>
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={() => navigate({ to: '/restaurants/pos/orders/new' })}>
            + New Order
          </Button>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {orders.map((o) => (
              <OrderRow
                ...
                order={o}
                onClick={() =>
                  navigate({ to: `/restaurants/pos/orders/$id`, params: { id: o.id } })
                }
              />
            ))}
          </div>
      ...
      <aside className="w-64 border-l p-4 overflow-y-auto">
        <div className="flex items-center justify-between mb-3">
          ...
          <Button
            size="sm"
            variant="ghost"
            onClick={() => navigate({ to: '/restaurants/pos/tables' })}
          >
            View All
          </Button>
        </div>
        ...
        <button
          ...
          onClick={() => {
            if (t.status === 'occupied') {
              ...
              navigate({
                to: '/restaurants/pos/orders/new',
                search: { tableId: t.id, type: 'dine-in' } as any,
              })
            }
          }}
          className={cn(
