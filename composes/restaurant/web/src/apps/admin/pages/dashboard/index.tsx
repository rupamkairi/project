    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          ...
          sub="Today"
        />
        <StatCard label="Orders" value={analytics.totalOrders ?? 0} sub={`Avg: `} />
        <StatCard
          label="Avg. Order Value"
          value={<AmountDisplay amount={analytics.avgOrderValue} />}
        />
        <StatCard label="Covers" value={analytics.covers ?? 0} sub="Dine-in guests" />
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {[
              ...
              { label: 'Pickup', count: analytics.pickup ?? 0 },
              { label: 'Partner', count: analytics.partnerDelivery ?? 0 },
            ].map((row) => (
              <div key={row.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-muted rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${analytics.totalOrders ? (row.count / analytics.totalOrders) * 100 : 0}%`,
                    }}
                  />
                </div>
