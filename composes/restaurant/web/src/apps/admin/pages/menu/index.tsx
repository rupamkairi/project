...
function MenuItemRow({
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-3 p-3 border-b last:border-0 transition-opacity',
    ...
    >
      ...
      <Switch checked={available} onCheckedChange={onToggle86} disabled={on86Pending} />
    </div>
  )
}
...
function MenuItemDialog({
  ...
  const save = useMutation({
    mutationFn: () =>
      isNew
        ? rstApi.createMenuItem({
            ...
            status: 'active',
            meta: {
              basePrice: String(parseFloat(form.price)),
              categoryId: form.categoryId,
              station: form.station,
            },
          } as any)
        : rstApi.updateMenuItem(item!.id, {
            name: form.name,
            meta: {
              basePrice: String(parseFloat(form.price)),
              categoryId: form.categoryId,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['rst-menu'] })
      onSaved()
      onClose()
    },
  })
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          ...
          <Input
            placeholder="Station (grill, cold, bar…)"
            value={form.station}
...
export function AdminMenuPage() {
  ...
  const toggle86 = useMutation({
    mutationFn: (item: MenuItem) =>
      rstApi.toggle86MenuItem(item.id, !(item.meta?.isAvailable !== false)),
      ...
      <div className="flex gap-2 overflow-x-auto">
        <button
          ...
          className={cn(
            ...
            !selectedCat ? 'bg-primary text-primary-foreground' : 'hover:bg-muted',
          )}
        >
          All ({items.length})
        </button>
        {cats.map((c) => (
          <button
            ...
            className={cn(
              'text-sm px-3 py-1 rounded-full border whitespace-nowrap',
              selectedCat === c.id ? 'bg-primary text-primary-foreground' : 'hover:bg-muted',
            )}
          >
            {c.name}
          </button>
