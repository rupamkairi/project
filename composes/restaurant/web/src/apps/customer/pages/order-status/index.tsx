        ...
        return (
          <React.Fragment key={step.key}>
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-xs mt-1 text-center max-w-14 ${active ? 'font-bold' : 'text-muted-foreground'}`}
              >
                {step.label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div
                className={`flex-1 h-0.5 mb-5 ${i < currentIndex ? 'bg-primary' : 'bg-border'}`}
              />
            )}
          </React.Fragment>
        )
      ...
      <div className="border rounded-xl divide-y">
        {(order.lines ?? []).map((line) => (
          <div key={line.id} className="flex justify-between text-sm px-4 py-3">
            <span>
              {line.qty}× {line.meta?.name ?? line.itemId}
            </span>
