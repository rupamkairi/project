  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      value: formData.value
        ? { amount: Number(formData.value), currency: formData.currency }
        : undefined,
    }
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-3 mt-1.5">
                ...
                <Badge variant={STATUS_VARIANTS[deal.status] ?? 'outline'}>
                  {deal.status ?? 'open'}
                </Badge>
              </div>
            </div>
          ...
          {stages.length > 0 && (
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {currentStageIdx >= 0 ? stages[currentStageIdx]?.name : 'No stage'} · Stage{' '}
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-1.5">
                      {stages.map((s, i) => (
                        <div
                          key={s.id}
                          className={`text-sm flex items-center gap-2 ${i === currentStageIdx ? 'font-medium text-foreground' : 'text-muted-foreground'}`}
                        >
                          <div
                            className={`h-2 w-2 rounded-full ${i < currentStageIdx ? 'bg-primary' : i === currentStageIdx ? 'bg-primary' : 'bg-muted-foreground/30'}`}
                          />
                          {s.name}
                        </div>
                      ))}
      ...
      <ConfirmDialog
        ...
        title="Mark as Won"
        description="Mark this deal as won. This updates the deal status."
      ...
      <Dialog open={loseOpen} onOpenChange={setLoseOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setLoseOpen(false)}>
              Cancel
            </Button>
            <Button size="sm" variant="destructive" onClick={handleLose}>
              Mark Lost
            </Button>
          </DialogFooter>
        </DialogContent>
