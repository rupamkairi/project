...
function DealCard({
  ...
  return (
    <div
      className="bg-card border rounded-lg p-3 space-y-2 cursor-pointer hover:shadow-sm transition-shadow"
      onClick={onClick}
    >
      ...
      <Badge variant={STATUS_VARIANTS[deal.status] ?? 'outline'} className="text-xs">
        {deal.status ?? 'open'}
      </Badge>
    </div>
  )
...
function DealsPage() {
  ...
  async function load(page = 1) {
    ...
    if (dealsRes.data) {
      ...
      setPagination(dealsRes.data.pagination)
    }
          ...
          {stages.length === 0 ? (
            ...
            stages.map((stage) => {
              ...
              const stageValue = stageDeals.reduce(
                (sum, d) => sum + Number(d.value?.amount ?? d.value ?? 0),
                0,
              )
              return (
                  ...
                  <div className="space-y-2">
                    {stageDeals.map((deal) => (
                      <DealCard
                        ...
                        onClick={() =>
                          navigate({ to: '/crm/deals/$dealId', params: { dealId: deal.id } })
                        }
                      />
                    ))}
                    {stageDeals.length === 0 && (
                ...
                deals.map((d) => {
                  ...
                  return (
                    <TableRow
                      ...
                      onClick={() =>
                        navigate({ to: '/crm/deals/$dealId', params: { dealId: d.id } })
                      }
                    >
                      <TableCell className="font-medium text-sm">{d.name ?? d.title}</TableCell>
                      <TableCell className="text-sm">
                        {d.value ? formatCurrency(d.value.amount ?? d.value) : '—'}
                      </TableCell>
