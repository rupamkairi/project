    ...
    const [person] = await db
      ...
      .where(
        and(
          ...
          eq(persons.organizationId, event.orgId),
  ...
  bus.on('campaign.email.opened', async (event) => {
    ...
    await Promise.all([
      ...
      db
        ...
        .where(
          and(
            eq(crmCampaignContact.campaignId, campaignId),
            eq(crmCampaignContact.personId, contactId),
          ),
        ),
    ...
    if (person) {
      ...
      await db
        .update(persons)
        .set({
          meta: { ...meta, leadScore: Math.min(100, (meta.leadScore ?? 0) + 5) },
          updatedAt: new Date(),
        })
