...
export function createAnalyticsRoutes(_mediator: Mediator) {
  return new Elysia({ prefix: '/analytics' })
    .get('/overview', async (ctx) => {
      ...
      const [[contacts], [accounts], [leads], [openDeals], [wonDeals], [activeCampaigns]] =
        await Promise.all([
          db
            .select({ value: count() })
            .from(persons)
          ...
          db
            .select({ value: count() })
            .from(crmLead)
            .where(and(eq(crmLead.organizationId, orgId), isNull(crmLead.deletedAt))),
          db
            .select({ value: count() })
            .from(crmDeal)
            .where(
          ...
          db
            ...
            .where(
              and(
                eq(crmCampaign.organizationId, orgId),
                eq(crmCampaign.status, 'sending'),
    ...
    .get('/activities/summary', async (ctx) => {
      ...
      const results = await db
        ...
        .from(activities)
        .where(and(eq(activities.organizationId, actor.orgId), isNull(activities.deletedAt)))
