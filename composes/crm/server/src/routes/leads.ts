...
import { assertLeadTransition, leadCanConvert, type LeadState } from '../lib/fsms'
  ...
  return (
    new Elysia({ prefix: '/leads' })
      .get('/', async (ctx) => {
        ...
        const leadConds = [eq(crmLead.organizationId, actor.orgId), isNull(crmLead.deletedAt)]
        ...
        const scoped = isManager(actor) ? leads : leads.filter((l) => l.ownerId === actor.id)
      ...
      .post('/:id/disqualify', async (ctx) => {
        ...
        const meta = {
          ...(lead.meta ?? {}),
          disqualificationReason: body.reason ?? 'No reason provided',
        }
        const [updated] = await db
          .update(crmLead)
      ...
      .post('/:id/convert', async (ctx) => {
        ...
        await db
          ...
          .set({
            type: 'contact',
            updatedAt: now,
            meta: { ...(lead.meta ?? {}), convertedAt: now.toISOString() },
          })
          .where(eq(persons.id, lead.personId))
