...
import { assertCampaignTransition, type CampaignState } from '../lib/fsms'
...
async function resolveSegmentContacts(
  ...
  const [segment] = await db
    ...
    .where(
      and(
        eq(crmSegment.id, segmentId),
        eq(crmSegment.organizationId, orgId),
  ...
  return (
    new Elysia({ prefix: '/campaigns' })
      .get('/', async (ctx) => {
        ...
        const conds = [eq(crmCampaign.organizationId, actor.orgId), isNull(crmCampaign.deletedAt)]
