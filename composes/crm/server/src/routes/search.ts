...
export function createSearchRoutes(_mediator: Mediator) {
  return new Elysia({ prefix: '/search' }).get('/', async (ctx) => {
        ...
        db
          ...
          .where(
            and(
              ...
              or(ilike(parties.name, pattern), ilike(parties.domain, pattern))!,
            ),
          )
          .limit(limit)
          .then((rows) => ({
      ...
      promises.push(
        (async () => {
          const leads = await db
            .select()
            .from(crmLead)
            .where(and(eq(crmLead.organizationId, actor.orgId), isNull(crmLead.deletedAt)))
            .limit(limit)
    ...
    const allResults = groups.flatMap((g) => g.results.map((r) => ({ ...r, entityType: g.entity })))
