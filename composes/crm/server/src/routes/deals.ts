  ...
  return (
    new Elysia({ prefix: '/deals' })
      ...
      .get('/pipeline/view', async (ctx) => {
        ...
        if (!pipelineFilter) {
          const [defaultPipeline] = await db
            ...
            .from(pipelines)
            .where(
              and(eq(pipelines.entityType, 'crm.deal'), eq(pipelines.organizationId, actor.orgId)),
            )
            .limit(1)
        ...
        const stages = await db
          ...
          .where(
            and(
              eq(pipelineStages.pipelineId, pipelineFilter),
      ...
      .get('/', async (ctx) => {
        ...
        const conds = [eq(crmDeal.organizationId, actor.orgId), isNull(crmDeal.deletedAt)]
      ...
      .get('/:id', async (ctx) => {
        ...
        const [party] = deal.partyId
          ? await db
              ...
              .from(parties)
              .where(eq(parties.id, deal.partyId))
              .limit(1)
          : [null]
