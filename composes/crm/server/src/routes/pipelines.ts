  ...
  return new Elysia({ prefix: '/pipelines' })
    .get('/', async (ctx) => {
      ...
      const items = await db
        .select()
        .from(pipelines)
        .where(and(eq(pipelines.organizationId, actor.orgId), isNull(pipelines.deletedAt)))

      // Filter to CRM entity types only
      const crmPipelines = items.filter((p) => p.entityType.startsWith('crm.'))
    ...
    .get('/:id', async (ctx) => {
      ...
      const stages = await db
        ...
        .from(pipelineStages)
        .where(
          and(eq(pipelineStages.pipelineId, id), eq(pipelineStages.organizationId, actor.orgId)),
        )
        .orderBy(pipelineStages.position)
