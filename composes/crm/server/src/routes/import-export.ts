...
function contactsToCsv(rows: any[]): string {
  const header =
    'id,firstName,lastName,email,phone,source,accountId,tags,status,leadScore,createdAt'
...
function dealsToCsv(rows: any[]): string {
  const header =
    'id,title,status,value,currency,probability,ownerId,stageId,pipelineId,expectedCloseDate,actualCloseDate,createdAt'
  const lines = rows.map((r) => {
    const val = r.value ?? {}
    return [
      r.id,
  ...
  return (
    new Elysia()
      ...
      .post('/import/contacts', async (ctx) => {
        ...
        const rawCsv: string = typeof body === 'string' ? body : (body.csv ?? body.file ?? '')
      ...
      .get('/export/contacts', async (ctx) => {
        ...
        ;(ctx as any).set.headers['Content-Disposition'] =
          `attachment; filename="contacts-${actor.orgId}.csv"`
        return csv
      })
      // --- Export: deals ------------------------------------------------------
      .get('/export/deals', async (ctx) => {
        ...
        ;(ctx as any).set.headers['Content-Type'] = 'text/csv'
        ;(ctx as any).set.headers['Content-Disposition'] =
          `attachment; filename="deals-${actor.orgId}.csv"`
        return csv
      })
  )
}
