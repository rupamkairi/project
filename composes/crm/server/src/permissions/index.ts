...
export const CRM_ANALYTICS_ROLES = [
  CRM_ROLES.admin,
...
export const CRM_PERMISSIONS = {
  ...
  // deals
  'deal:read': [CRM_ROLES.admin, CRM_ROLES.salesManager, CRM_ROLES.salesRep, CRM_ROLES.viewer],
  ...
  // pipelines
  'pipeline:read': [CRM_ROLES.admin, CRM_ROLES.salesManager, CRM_ROLES.salesRep, CRM_ROLES.viewer],
  'pipeline:manage': [CRM_ROLES.admin, CRM_ROLES.salesManager],

  // campaigns
  'campaign:read': [CRM_ROLES.admin, CRM_ROLES.salesManager, CRM_ROLES.marketing, CRM_ROLES.viewer],
...
export function requirePermission(
  ...
  if (
    canAccess(actor, permission, { composeAdminRoles: COMPOSE_ADMIN_ROLES.crm }) ||
...
export function isManager(actor: AuthActor): boolean {
  return (CRM_MANAGER_ROLES as readonly string[]).some((r) => actor.roles.includes(r))
}
