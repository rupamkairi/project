...
import {
  ...
  ecoShippingOptions,
  ecoTaxRates,
} from '@projectx/ecommerce-server/db/schema'
...
const DEFAULT_TAX_RATES: Record<string, { name: string; rate: number; jurisdiction?: string }[]> = {
  'US Standard': [{ name: 'Federal Tax', rate: 0, jurisdiction: 'Federal' }],
  'EU VAT': [
...
const DEFAULT_REGIONS = [
  ...
  },
  {
    ...
    currency: 'INR',
    countries: ['IN'],
    taxProfileName: 'India GST',
    isDefault: false,
  },
]

const DEFAULT_SHIPPING_OPTIONS: Record<
  string,
  { name: string; type: string; rate: number; estimatedDays: number }[]
> = {
  'United States': [
  ...
  'European Union': [{ name: 'Standard Shipping', type: 'flat_rate', rate: 499, estimatedDays: 7 }],
  India: [
