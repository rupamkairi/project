...
export function createProductsRoutes(mediator: Mediator) {
  return new Elysia({ prefix: '/products' })
    .get('/', async ({ query }: any) => {
      ...
      return {
        data: results.map((r: any) => toProduct(r)),
    ...
    .post('/:id/variants', async ({ params, body }: any) => {
      ...
      const result = await db
        .insert(catVariants)
        .values({ ...row, id: crypto.randomUUID(), organizationId: '', meta: {}, version: 1 })
        .returning()
      return result[0]
    })
    .get('/:id/variants/:variantId', async ({ params }: any) => {
      const result = await db
        .select()
        .from(catVariants)
        .where(and(eq(catVariants.id, params.variantId), eq(catVariants.itemId, params.id)))
        .limit(1)
      if (!result[0]) return null
      return {
        ...result[0],
        options: result[0].attributes ?? {},
        stockQty: 0,
        price: 0,
        compareAtPrice: null,
      }
    ...
    .patch('/:id/variants/:variantId', async ({ params, body }: any) => {
      ...
      if (body.options)
        update.attributes =
          typeof body.options === 'string' ? JSON.parse(body.options) : body.options
