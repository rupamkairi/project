  ...
  if (!customer) {
    return (
      <div className="mx-auto max-w-sm px-4 py-16 text-center space-y-4">
        ...
        <Button asChild>
          <Link to="/ecommerce/store/auth/login">Sign In</Link>
        </Button>
        <p className="text-sm text-muted-foreground">
          Don't have an account?{' '}
          <Link
            to="/ecommerce/store/auth/register"
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: 'Active',
            value: orders.filter((o: any) => o.status === 'processing' || o.status === 'pending')
              .length,
            icon: ShoppingBag,
          },
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {orders.length > 0 && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate({ to: '/ecommerce/store/account' })}
            >
              ...
              <tbody>
                {orders.map((o: any) => (
                  <tr key={o.id} className="border-t hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3">
                      <Badge className={STATUS_BADGES[o.status] ?? ''}>{o.status}</Badge>
                    </td>
                    ...
                    <td className="p-3 text-right">
                      <Button
                        ...
                        onClick={() =>
                          navigate({
                            to: '/ecommerce/store/account/orders/$id',
                            params: { id: o.id },
                          })
                        }
                      >
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Link to="/ecommerce/store/account" className="hover:text-foreground transition-colors">
          Account
        </Link>
        <span>/</span>
              ...
              {items.map((i: any) => (
                <tr key={i.id} className="border-b last:border-0">
                  ...
                  <td className="py-3 text-right">
                    {formatCurrency(i.unitPrice?.amount ?? i.unitPrice)}
                  </td>
