...
function ProductForm({ product, onClose }: { product?: any; onClose: () => void }) {
  ...
  const { data: catData } = useQuery({
    ...
    queryFn: () => ecommerceAdminApi.getCategories(),
  })
  ...
  const mutation = useMutation({
    mutationFn: () =>
      product
        ? ecommerceAdminApi.updateProduct(product.id, form)
        : ecommerceAdminApi.createProduct(form),
    onSuccess: () => {
...
function AdminProducts() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => ecommerceAdminApi.deleteProduct(id),
  ...
  const columns = [
    ...
    { header: 'Handle', accessorKey: 'handle' },
    {
      header: 'Status',
      cell: ({ row }: any) => (
        <Badge className={STATUS_BADGES[row.original.status]}>{row.original.status}</Badge>
      ),
    },
