...
function VariantDialog({
  ...
  const [form, setForm] = useState({
    ...
    stockQty: variant?.stockQty ? String(variant.stockQty) : '0',
  ...
  const mutation = useMutation({
    mutationFn: () =>
      variant
        ? ecommerceAdminApi.updateVariant(productId, variant.id, form)
        : ecommerceAdminApi.createVariant(productId, form),
    onSuccess: () => {
    ...
    <DialogContent>
      ...
      <DialogFooter>
        ...
        <Button
          ...
          disabled={!form.sku || !form.price || mutation.isPending}
        >
...
function AdminProductDetail() {
  ...
  const { data: productData } = useQuery({
    ...
    queryFn: () => ecommerceAdminApi.getProduct(id),
  })
  const { data: variantsData } = useQuery({
    queryKey: ['admin-product-variants', id],
    queryFn: () => ecommerceAdminApi.getVariants(id),
  })
  ...
  const deleteVarMut = useMutation({
    mutationFn: (vid: string) => ecommerceAdminApi.deleteVariant(id, vid),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          { label: 'Products', href: '/ecommerce/admin/products' },
          { label: product?.title ?? '' },
        ]}
        actions={
          product && (
                      ...
                      <tr key={v.id} className="border-b last:border-0">
                        ...
                        <td className="p-3 flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => deleteVarMut.mutate(v.id)}
                          >
