...
function AdminReturns() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => ecommerceAdminApi.getReturns(),
  })
            ...
            <tbody>
              {returns.map((r: any) => (
                <tr
                  ...
                  onClick={() =>
                    navigate({ to: '/ecommerce/admin/returns/$id', params: { id: r.id } })
                  }
                >
                  <td className="p-3 font-medium">
                    {r.order?.referenceNo ?? r.orderId?.slice(0, 8) ?? '—'}
                  </td>
          ...
          {ret.orderId && (
            <p>
              ...
              <span className="font-medium">
                {ret.order?.referenceNo ?? ret.orderId.slice(0, 8)}
              </span>
            </p>
          )}
