...
function AdminFulfillment() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-fulfillments', statusFilter],
    queryFn: () => ecommerceAdminApi.getFulfillments({ status: statusFilter }),
  })
  const updateMut = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      ecommerceAdminApi.updateFulfillmentStatus(id, status),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-fulfillments'] }),
  })

  const fulfillments = data?.data?.data ?? []
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">No fulfillments found</p>
      ) : (
              ...
              {fulfillments.map((f: any) => (
                <tr key={f.id} className="border-b last:border-0 hover:bg-muted/50">
                  <td
                    ...
                    onClick={() =>
                      navigate({ to: '/ecommerce/admin/orders/$id', params: { id: f.orderId } })
                    }
                  >
                  ...
                  <td className="p-3">
                    <Badge className={STATUS_BADGES[f.status] ?? ''}>{f.status}</Badge>
                  </td>
                  <td className="p-3">
                    {f.status === 'pending' && (
                      <div className="flex gap-2">
                        <Button
                          size="sm"
                          onClick={() => updateMut.mutate({ id: f.id, status: 'shipped' })}
                        >
                          Mark Shipped
                        </Button>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => updateMut.mutate({ id: f.id, status: 'delivered' })}
                        >
                          Mark Delivered
                        </Button>
                      </div>
