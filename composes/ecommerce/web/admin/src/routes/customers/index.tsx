...
import { PageHeader, Input, Button, Avatar, AvatarFallback } from '@projectx/ui'
...
function AdminCustomers() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => ecommerceAdminApi.getCustomers(q ? { q } : {}),
  })
            ...
            <tbody>
              {customers.map((c: any) => (
                <tr
                  ...
                  onClick={() =>
                    navigate({ to: '/ecommerce/admin/customers/$id', params: { id: c.id } })
                  }
                >
                  <td className="p-3 flex items-center gap-2">
                    <Avatar size="sm">
                      <AvatarFallback>
                        {(c.person?.name ?? c.email ?? '?').slice(0, 1).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
...
function AdminCustomerDetail() {
  ...
  const { data: custData } = useQuery({
    ...
    queryFn: () => ecommerceAdminApi.getCustomer(id),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={cust?.person?.name ?? 'Customer'}
        breadcrumbs={[{ label: 'Customers', href: '/ecommerce/admin/customers' }]}
      />
      {cust && (
        <>
          <div className="flex items-center gap-3">
            <Avatar>
              <AvatarFallback>
                {(cust.person?.name ?? cust.email ?? '?').slice(0, 1).toUpperCase()}
              </AvatarFallback>
            </Avatar>
            <div>
